...
```{r, fig.width=10, fig.height=5}
# Plot histogram of hte ABV distribution
clean_df %>%
  ggplot(aes(x = ABV)) +
...
#Plot 
clean_df%>% filter(State=="ID" | State=="MI") %>% select(ABV,IBU,State)%>% ggplot(aes(x= ABV, y=IBU, color =State))+ geom_point() +ggtitle("Indiana and Miami ABV/IBU")
