...
# Google API Key
ggmap::register_google(key = "AIzaSyCqBihKrMc1DZxxFLD9z8XdQe-Ub3HuaKc")
...
# Fix the longitude in the cities_df dataframe
cities_df$longitude <- as.double(cities_df$longitude)  
cities_df <- cities_df %>% filter(!is.na(longitude))

# Plot the US map with number of craft breweries and Budweiser locations
map_w_brew <- ggplot(data = brew_by_state, aes(map_id = id)) + 
  geom_map(aes(fill = count),  color= "white", map = fifty_states) + 
  expand_limits(x = fifty_states$long, y = fifty_states$lat) +
  coord_map() + 
  ggtitle("Number of Breweries by State with Anheuser-Busch Brewery Locations") +
